...
.controller('IngresoMenuCtrl', ['$scope', '$window', '$location', 'ngTableParams', '$filter', 'Entidad', '$routeParams', '$rootScope','$cookieStore', 'Administracion', '$localStorage',
  function($scope, $window, $location, ngTableParams, $filter, Entidad, $routeParams, $rootScope, $cookieStore, Administracion, $localStorage) {
      ...
      $scope.guardar = function(objeto){
        ...
        if(objeto.mendescripcion && objeto.menpadre){
      ...
      $scope.validarRepetidos = function(objeto){
        if(objeto.mendescripcion){
          Administracion.validarRepetidos($scope.tabla, 'mendescripcion', objeto.mendescripcion,
          function(result){
            if(!result){
              objeto.mendescripcion = '';
              $scope.error = true;
              $scope.textoError = 'EL MENU INGRESADO YA EXISTE';
            }else{
              $scope.error = false;
              $scope.textoError = '';
            }
